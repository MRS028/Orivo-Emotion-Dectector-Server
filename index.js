...
// ==== MongoDB Connection ====
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/tour_management";
mongoose
  .connect(MONGODB_URI)
...
app.get("/api/emotions", async (req, res) => {
  try {
    ...
    const emotions = await Emotion.find({ userId: user._id }).sort({
      createdAt: -1,
    });
...
app.post("/api/emotions", async (req, res) => {
  ...
  try {
    ...
    if (!email || !text || !detectedEmotion) {
      return res
        .status(400)
        .json({ error: "Email, text and detectedEmotion are required" });
    }
